...
namespace BUS_QL_BanGiay
{
    public class GiayBUS
    {
        ...
        {
            List<GiayDTO> listGiay = new List<GiayDTO>();
            DataTable dt = _giayDAL.LoadGiaySapXepTang(); // 👈 gọi DAL tăng dần
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                {
                    decimal tyLeGiam = phanTramGiam / 100m;
                    giaSauUuDai = Math.Ceiling(donGia * (1m - tyLeGiam));
                }
        ...
        {
            List<GiayDTO> listGiay = new List<GiayDTO>();
            DataTable dt = _giayDAL.LoadGiaySapXepGiam(); // 👈 gọi DAL giảm dần
        ...
        //Hieu
        public List<GiayDTO> GetAllADKM()
        {
            return giayDAL.GetAllGiayADKM();
