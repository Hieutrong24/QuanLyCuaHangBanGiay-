    ...
    public partial class BanHangCuaNhanVien : Form
    {
        ...
        private void LoadGiay()
        {
            ...
            foreach (var giayDTO in danhSachGiay)
            {
                ...
                string imagePath = Path.Combine(imageFolder, giayDTO.Images);
                Image anhGiay = null;
        ...
        {
            // 1. Tổng cộng (decimal cho chính xác tiền tệ)
            decimal tongGiaTri = 0;

            // Tên cột trong DataGridView
            string tenCotGia = "Gia";       // Cột đơn giá
            string tenCotSoLuong = "SoLuong"; // Cột số lượng

            // 2. Nếu không có dữ liệu thì gán 0 luôn
            if (dtgrv.Rows.Count == 0)
            {
                lblTongGia.Text = "Tổng: 0 VNĐ";
                return;
            }
            ...
            foreach (DataGridViewRow row in dtgrv.Rows)
            {
                ...
                // Lấy giá trị cột "Gia" và "SoLuong"
                object cellGia = row.Cells[tenCotGia].Value;
                object cellSoLuong = row.Cells[tenCotSoLuong].Value;

                if (cellGia == null || cellGia == DBNull.Value || cellSoLuong == null || cellSoLuong == DBNull.Value)
                    continue; // Bỏ qua hàng thiếu dữ liệu

                // Chuyển đổi sang số
                decimal gia, soLuong;

                if (decimal.TryParse(cellGia.ToString(), out gia) && decimal.TryParse(cellSoLuong.ToString(), out soLuong))
                {
                    // ✅ Tổng tiền mỗi hàng = giá * số lượng
                    tongGiaTri += gia * soLuong;
                }
                else
                {
                    // Nếu dữ liệu sai định dạng
                    MessageBox.Show($"Lỗi dữ liệu ở hàng {row.Index + 1}. Vui lòng kiểm tra lại giá hoặc số lượng.",
                                    "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            giahoadon = tongGiaTri;
            // 4. Hiển thị tổng cộng
            lblTongGia.Text = $"{tongGiaTri:N0} VNĐ"; // Định dạng có dấu phẩy ngăn cách
        }
        ...
        private void TimGiay_Click(object sender, EventArgs e)
        {
        ...
        private void btnTimMaGiay_Click(object sender, EventArgs e)
        {
            string text = SearchBox.Text.Trim();
            allCards.Clear();

            if (long.TryParse(text, out long maGiay))
            {
                List<GiayDTO> danhSachGiay = _giayBUS.LoadGiayTheoMa(maGiay);

                if (danhSachGiay.Count == 0)
                {
                    MessageBox.Show("Không tìm thấy giày có mã: " + maGiay);
                    return;
                }
            ...
            {
                MessageBox.Show("Mã giày không hợp lệ! Hãy nhập số nguyên (hoặc quét mã QR).");
            }
        }
        ...
        private void uiButton1_Click(object sender, EventArgs e)
        {
            ...
            HoaDon formMoi = new HoaDon(dt,giahoadon);
            formMoi.Show();
        ...
        private void uiComboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (index)
            {
                case 0:
                    ...
                    List<GiayDTO> danhSachGiay2 = _giayBUS.LoadGiayVaApDungKhuyenMai_Tang();

                    foreach (var giayDTO in danhSachGiay2)
                    {
                        string basePath = Path.GetFullPath(Path.Combine(Application.StartupPath, @"..\..\.."));
                        string imageFolder = Path.Combine(basePath, "Images");
                        string imagePath = Path.Combine(imageFolder, giayDTO.Images);
                ...
                case 1:
                    ...
                    List<GiayDTO> danhSachGiay1 = _giayBUS.LoadGiayVaApDungKhuyenMai_Giam();

                    foreach (var giayDTO in danhSachGiay1)
                    {
                        string basePath = Path.GetFullPath(Path.Combine(Application.StartupPath, @"..\..\.."));
                        string imageFolder = Path.Combine(basePath, "Images");
                        string imagePath = Path.Combine(imageFolder, giayDTO.Images);
        ...
        {
            MainForm frmQR = new MainForm();

            // Đăng ký sự kiện nhận mã từ form quét QR
            frmQR.OnQRCodeScanned += (maQR) =>
            {
                // Khi form quét gửi mã về, gán vào ô tìm kiếm
                this.Invoke(new Action(() =>
                {
                    SearchBox.Text = maQR;
                    // Tự động tìm giày
                    btnTimMaGiay_Click(sender, e);
                }));
            };
