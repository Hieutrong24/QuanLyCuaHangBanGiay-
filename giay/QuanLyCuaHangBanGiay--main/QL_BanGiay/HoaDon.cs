...
namespace QL_BanGiay
{
    ...
    public partial class HoaDon : Form
    {
        ...
        decimal giahd;
        public HoaDon(DataTable dt, decimal giahd)
        {
            InitializeComponent();
            this.giahd = giahd;
            this.dt = dt;
        ...
        private void TinhTongVaHienThi(DataGridView dtgrv, Label lblTongGia)
        {
            ...
            lblthue.Text = "Thuế (10% giá trị đơn hàng) : " + giahd * (decimal)0.1;
            lblTongGia.Text = "Tổng giá đơn hàng : " + $"{giahd + tongGiaTri * (decimal)0.1:N0} VNĐ";
        ...
        private void uiButton2_Click(object sender, EventArgs e)
        {
            if (check == false)
            {
                MessageBox.Show("Chưa kiểm tra mã khách hàng.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (txtMaKH.Text.Length > 0)
            {
                if (cotk == false)
                {
                    ...
                    {
                        MessageBox.Show("Mã khách hàng không hợp lệ (phải là số điện thoại).", "Lỗi nhập liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                ...
                foreach (DataGridViewRow row in uiDataGridView1.Rows)
                {
                    ...
                    giayBUS.CapNhatSoLuong(maGiay, soLuong);
