...
namespace QL_BanGiay
{
    ...
    {
        DateTime TN=DateTime.Now, DN=DateTime.Now;
        string ht, nq;
        public frmTinhLuong()
        {
        ...
        private void btnTinhLuong_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in data_TinhLuong.Rows)
            {
                if (row.Cells["clmLuong1NC"].Value != null)
                {
                    decimal luong1NC = Convert.ToDecimal(row.Cells["clmLuong1NC"].Value);
                    int snc = TinhNgayCong(DN, TN);
                    decimal luong = luong1NC * snc;
                    row.Cells["clmSNC"].Value = snc;
                    row.Cells["clmLuong"].Value = Math.Round(luong, 2);
                    // Tính thưởng
                    int thuong = 0;
                    int.TryParse(cboThuong?.SelectedValue?.ToString(), out thuong);
                    decimal tienThuong = luong * thuong / 100;
                    row.Cells["clmThuong"].Value = Math.Round(tienThuong, 2);
                    // Tính tổng lương sau thưởng
                    decimal tongLuong = luong + tienThuong;
                    // Tính phạt
                    int phat = 0;
                    int.TryParse(cboPhat?.SelectedValue?.ToString(), out phat);
                    decimal tienPhat = tongLuong * phat / 100;
        ...
        private void cboNQ_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            if(cboNQ.SelectedIndex != -1)
            {
